...
// Hook to count up numbers (skills)
...
const About = () => {
  ...
  return (
    ...
    >
      <div className="relative z-10 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16 items-start">
        ...
        <div className={`space-y-8 transition-all duration-1000 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          ...
          <div className="space-y-4">
            {skills.map((skill, i) => {
              const count = useCountUp(skill.level);
              return (
                <div key={i}>
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            {features.map((item, index) => (
              <div
                key={item.title}
                className={`group relative rounded-2xl p-6 bg-slate-900/50 backdrop-blur-md border border-slate-700 hover:border-gradient-to-r hover:from-sky-400 hover:via-purple-500 hover:to-pink-400 shadow-md hover:shadow-xl hover:-translate-y-2 transition-all duration-500 opacity-0 animate-fade-up delay-${700 + index*200}`}
              >
      ...
      {/* Journey Timeline */}
      <div className={`max-w-3xl mx-auto px-6 md:px-12 py-24 transition-all duration-1000 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
